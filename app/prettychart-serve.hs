...
data Run = RunWatch | RunDemo | RunDataFrame | RunPush | RunMouse deriving (Eq, Show)
...
parseRun =
  flag' RunDemo (long "demo" <> help "run interactive server demo")
    ...
    <|> flag' RunDataFrame (long "dataframe" <> help "run dataframe chart push test")
...
pushServer :: (Int -> ChartOptions) -> AppConfig -> IO ()
  ...
  let app :: Application
      app _request respond = do
        case pathInfo _request of
          ["sse"] -> streamFrames respond (encodeChartOptions . anim) numFrames frameDelay
...
-- | Load CSV data from test file
getDF :: IO D.DataFrame
getDF = D.readCsv "other/s5e11/test.csv"

-- | Generate composite box plot chart from all numeric columns
multiBoxPlot :: D.DataFrame -> [String] -> ChartOptions
multiBoxPlot df dbls = c
  where
    -- Use all columns as Double (will error if column isn't numeric)
    -- Extract data lists for each column
    xss = (\c -> D.columnAsList @Double (F.col c) df) <$> (pack <$> dbls)
    -- Create box plots
    cs = boxPlot defaultBoxPlotOptions <$> xss
    -- Add column names as titles
    boxCharts = zipWith (\c t -> c & over #hudOptions (addTitle PlaceBottom 0.03 t)) cs (pack <$> dbls)
    -- Compose horizontally
    c = horiCO AlignMid 0.1 boxCharts

multiPlot :: D.DataFrame -> [String] -> Int -> ChartOptions
multiPlot df dbls i = multiBoxPlot (D.take (i+10) df) dbls
...
main = do
  ...
  case r of
    ...
    RunDataFrame -> do
      df <- getDF
      pushServer (multiPlot df ["loan_amount", "interest_rate", "debt_to_income_ratio", "annual_income"]) o
    RunPush -> pushServer counterChart o
