...
-- Count occurrences of values in a list, sorted descending by frequency
count :: [Text] -> [(Text, Int)]
count vals =
  vals
    |> foldl' (\m v -> Map.insertWith (+) v 1 m) Map.empty
    |> Map.toList
    |> sortOn (Down . snd)

---- Create a stacked bar chart from counted values
countedBar :: Text -> [(Text, Int)] -> ChartOptions
countedBar title counts =
  counts
    |> fmap (\(label, freq) -> (label, fromIntegral freq))
    |> stackedBar defaultStackedBarOptions title

-- Extract column as [Text], count, and chart
columnToCount :: Text -> D.DataFrame -> [(Text, Int)]
columnToCount colName df =
  df
    |> D.columnAsList @Text (F.col colName)
    |> count

-- Extract column as [Text], count, and chart
columnToChart :: D.DataFrame -> Text -> ChartOptions
columnToChart df colName =
  df
    |> columnToCount colName
    |> countedBar colName
...
catCount :: Text -> D.DataFrame -> [(Text, Int)]
catCount colName df = chartData
  where
    vals = D.columnAsList @Text (F.col colName) df
          -- Count occurrences using Map
    counts = foldl' (\m v -> Map.insertWith (+) v (1::Int) m) Map.empty vals
          -- Sort by count descending
    sorted = sortOn (Down . snd) (Map.toList counts)
    ...
    top10 = take 10 sorted
         -- Convert to [(Text, Double)] format
    chartData = [(fst pair, (snd pair)) | pair <- top10]
